...
def main(camera_index=0):
    ...
    try:
        ...
        while True:  # Keep running until user quits or an error occurs
            ...
            # ret = True if frame was captured successfully, False if failed
            # frame = the actual image/video frame from the webcam
            ...
            cv2.putText(frame, "Press 'q' to quit", (10, 30),  # Position: (10,30) from top-left corner
                        ...
                        (0, 255, 0), 2)                        # Green color (BGR format), thickness = 2 pixels
            
            cv2.imshow("Webcam Test", frame)  # Display the frame in a window named "Webcam Test"
            
            '''
            Check for key press:
            cv2.waitKey(1) waits for 1 millisecond for a key press.
            & 0xFF extracts only the last 8 bits (standard key codes).
...
It handles command-line arguments and starts the main function.
'''
if __name__ == "__main__":
    ...
    Example usage:
    python test_webcam.py     # Uses camera index 0 (default)
    python test_webcam.py 1   # Uses camera index 1
    python test_webcam.py 2   # Uses camera index 2
    '''
    index = int(sys.argv[1]) if len(sys.argv) > 1 else 0  # Use first argument if given, else default to 0
    # sys.argv[1] is the first command-line argument after the script name
    # If no argument is provided, use camera index 0
