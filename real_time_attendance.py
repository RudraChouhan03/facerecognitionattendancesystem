# Import all necessary libraries that this program needs to work
...
RE_LOG_GAP = 60  # Minimum seconds between logging the same person's attendance again
MIN_CONFIDENCE = 0.40  # Threshold for face recognition confidence (lower is stricter)
...
LIVENESS_THRESHOLD = 0.65  # Threshold for deciding if a face is real or spoofed
...
Think of encodings as unique "fingerprints" for each person's face.
'''
# Load face encodings (the unique numbers representing each known person's face)
try:
...
def initialize_camera():
    '''
    This function tries to connect to available cameras (webcam, USB, or CCTV).
    It checks multiple camera indices and returns the first working camera.
    '''
    # ============= CCTV INTEGRATION =============
    # The following lines are commented out. They show how to connect to CCTV/IP cameras if needed.
    # RTSP (IP Camera)
    # cctv_rtsp_url = "rtsp://username:password@192.168.1.100:554/stream1"
    # cap = cv2.VideoCapture(cctv_rtsp_url)
    # if cap.isOpened():
    #     print("[INFO] Successfully connected to CCTV via RTSP")
    ...
    #     return cap
    # HTTP/MJPEG
    # cctv_http_url = "http://192.168.1.100:8080/video"
    # cap = cv2.VideoCapture(cctv_http_url)
    # if cap.isOpened():
    #     print("[INFO] Successfully connected to CCTV via HTTP")
    #     return cap
    # USB Camera
    # for idx in [0, 1, 2, 3]:
    ...
    #             print(f"[INFO] USB CCTV camera found at index {idx}")
    #             cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
    #             cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
...
'''
# Initialize liveness detector (checks if face is real or a photo)
...
while True:
    ...
    # ============= CCTV FRAME PROCESSING OPTIMIZATION =============
    # The following lines are commented out. They show how to optimize processing for CCTV by skipping frames or focusing on a detection zone.
    # FRAME_SKIP = 5  # Adjust as needed
    # if frame_count % FRAME_SKIP != 0:
    #     continue
    # DETECTION_ZONE = (100, 100, 500, 400)
    # if DETECTION_ZONE is not None:
    #     x1, y1, x2, y2 = DETECTION_ZONE
    #     cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 0, 0), 2)   ## Draws a blue rectangle (since (255, 0, 0) is blue in BGR color format) on your video frame from the point (x1, y1) (top-left corner) to (x2, y2) (bottom-right corner). The thickness of the rectangle border is 2 pixels.
    #     cv2.putText(frame, "DETECTION ZONE", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)  ## Writes the text "DETECTION ZONE" just above the rectangle, at position (x1, y1-10), using the same blue color and a readable font and size
    ...
    if not boxes:
        ...
        for i, (startX, startY, endX, endY) in enumerate(boxes):
            ...
            text_color = (255, 255, 255) if name == "Unknown" else (0, 0, 0)  # White text for unknown, black for known
    ...
    cv2.putText(frame, f"Frame: {frame_count}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)  # Show frame count
    if ENABLE_LIVENESS_DETECTION:
    ...
    cv2.putText(frame, "POLICY: 1 Entry + 1 Exit per day", (10, 90),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)  # Show attendance policy
    cv2.putText(frame, "Press 'q' to quit, 'l' to toggle liveness, 'd' for debug", (10, frame.shape[0] - 10),
    ...
    if key == ord('q'):
        ...
        DEBUG_LIVENESS = not DEBUG_LIVENESS  # Toggle debug messages for liveness
        print(f"[INFO] Liveness debug {'ENABLED' if DEBUG_LIVENESS else 'DISABLED'}")

'''
Program cleanup:
This section closes the camera and windows when the program ends.
'''
